...
#Initilization of root and base widgets
    def __init__(self, master):
        ...
        # Landing page canvas
        self.canvas_main = Canvas(master=self.frame, width=740, height=710)
        self.canvas_main.grid(row=0, column=0, columnspan=2, rowspan=2, sticky="nsew")

        with open("C:\\Users\\alext\OneDrive\\Desktop\Visual Studio Code\\URL Toolkit Folder\\Mainpage.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.maincanvas_pic = ImageTk.PhotoImage(Image.open(image_bytes))

        self.image_references.append(self.maincanvas_pic)
        self.image_on_canvas = self.canvas_main.create_image(370, 355, image=self.maincanvas_pic, anchor=tkinter.CENTER)
        ...
        with open("URL Toolkit Folder\Question Mark.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            img = Image.open(image_bytes)
            img = img.resize((30,30))
            self.Q_pic = ImageTk.PhotoImage(img)
            self.image_references.append(self.Q_pic)


        #Example elements for flyer method
        with open("URL Toolkit Folder\Dark Flyer Example.PNG", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.dark_example_f = Image.open(image_bytes)
            self.dark_example_f = self.dark_example_f.resize((125, 175))
            self.dark_example_f = ImageTk.PhotoImage(self.dark_example_f)
        self.image_references.append(self.dark_example_f)


        with open("URL Toolkit Folder\Blue Image Example.PNG", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.blue_example_f = Image.open(image_bytes)
            self.blue_example_f = self.blue_example_f.resize((125, 175))
            self.blue_example_f = ImageTk.PhotoImage(self.blue_example_f)
        self.image_references.append(self.blue_example_f)


        with open("URL Toolkit Folder\Orange Flyer Example.PNG", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.orange_example_f = Image.open(image_bytes)
            self.orange_example_f = self.orange_example_f.resize((125, 175))
            self.orange_example_f = ImageTk.PhotoImage(self.orange_example_f)
        self.image_references.append(self.orange_example_f)
    ...
    def shorten_help(self):
        ...
        with open("URL Toolkit Folder\\URL Shortener Helpful Tips.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.shortenhelp_pic = ImageTk.PhotoImage(Image.open(image_bytes))
        
        self.image_references.append(self.shortenhelp_pic)
    ...
    def QR_help(self):
        ...
        with open("URL Toolkit Folder\QR Generator Helpful Tips.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.QRhelp_pic = ImageTk.PhotoImage(Image.open(image_bytes))        
        self.image_references.append(self.QRhelp_pic)
    ...
    def flyer_help(self):
        ...
        with open("URL Toolkit Folder\Flyer Helpful Tips.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.flyerhelp_pic = ImageTk.PhotoImage(Image.open(image_bytes))        
        self.image_references.append(self.flyerhelp_pic)
        self.flyerhelp_on_canvas = self.canvas_fhelp.create_image(450, 250, image=self.flyerhelp_pic, anchor=tkinter.CENTER)
    ...
    def orange_template(self):
        with open("URL Toolkit Folder\Orange & White Elegant Business Flyer Potrait (2).png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.ot_img = Image.open(image_bytes)
            self.ot_img.save("ot_img.png")
        self.image_references.append(self.ot_img)

        with open("URL Toolkit Folder\Orange Blank.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.orange_blank = Image.open(image_bytes)
            self.orange_blank = self.orange_blank.resize((500,500))
        self.image_references.append(self.orange_blank)
    ...
    def blue_template(self):
        with open("URL Toolkit Folder\Light Blue Purple Simple Gradient Travel Poster (2).png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        with BytesIO(base64.b64decode(encoded_string)) as image_bytes:
            self.bt_img = Image.open(image_bytes)
            self.bt_img.save("bt_img.png")
        self.image_references.append(self.bt_img)
